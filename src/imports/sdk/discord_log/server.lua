...
--- Discord webhook message builder (server-side). `M.createDiscordLog(url, botName [, opts])` -> builder object;
...
--- Trims leading and trailing whitespace via shared helper base.
...
function M.createDiscordLog(webhook, botName, opts)
    ...
    if not isValidWebhookUrl(webhook) then
        cLogFn('[e_core][DiscordLog]', 'invalid webhook URL (expected https://.../api/webhooks/...)', 1)
        return false
    end
...
--- Fills one embed from a table payload. `newEmbed=true` appends a new embed.
...
--- Quick status embed based on `level`.
--- @param opts table|nil
--- @return table
...
function DiscordLog:send(doneCb)
    ...
    if not hasContent and not hasEmbeds then
        cLogFn('[e_core][DiscordLog]', 'send skipped: empty content and no embeds', 2)
        if doneCb then
    ...
    PerformHttpRequest(self._webhook, function(statusCode, data, _headers)
        ...
        if not ok then
            ...
            cLogFn(('[e_core][DiscordLog] HTTP %s'):format(tostring(statusCode)), snippet, 1)
