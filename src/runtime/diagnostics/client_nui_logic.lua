...
--- Handles diagnostics RPC result event and resolves pending callback.
--- @param requestId string|number
--- @param result table|nil
--- @return nil
local function onNuiDiagnosticsRpcResult(requestId, result)
...
--- Handles diagnostics NUI API callback and forwards request to server RPC.
--- @param data table|nil
--- @param cb function
--- @return nil
local function onNuiDiagnosticsApi(data, cb)
    seq = seq + 1
...
return {
    onNuiDiagnosticsRpcResult = onNuiDiagnosticsRpcResult,
    onNuiDiagnosticsApi = onNuiDiagnosticsApi,
}
